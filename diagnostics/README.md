...
| `probe4_peace_syntax.ps1` | How a script asks for peace. Answer: `CeaseFire With "X"` and `Peace Treaty With "X"` both work and both produce a ceasefire of the default length. `Peace "X"`, `Peace Treaty "X"`, `Peace With "X"` and any trailing duration are parse errors. Note the parser stops at the first bad file, so forms after an error are never reached — verify one thing at a time. |
